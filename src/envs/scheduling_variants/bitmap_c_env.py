...
旧ビットマップ実装のバックアップ: ``src/envs/backup_bitmap/`` （モジュール: ``scheduling_variants.bitmap_c_env``）
"""
import numpy as np
import sys
import os
...
# scheduling_env_core の find_allocation_position はビルド世代により
# 5引数版（continuous_only なし）と 6引数版がある
_FIND_ALLOCATION_USE_CONTINUOUS_ARG = None


def _call_find_allocation_position(cache_c, job_width, job_height, when_submitted, current_time, use_cloud):
    global _FIND_ALLOCATION_USE_CONTINUOUS_ARG
    if _FIND_ALLOCATION_USE_CONTINUOUS_ARG is False:
        return c_find_allocation_position(
    ...
    try:
        ...
        if "incompatible function arguments" not in msg and "takes" not in msg:
            raise
        _FIND_ALLOCATION_USE_CONTINUOUS_ARG = False
        import warnings

        warnings.warn(
            "scheduling_env_core.find_allocation_position は5引数版です。"
            "continuous_only なしで実行します。"
            "推奨: pip install -e src/envs/scheduling_native で再ビルド",
            UserWarning,
            stacklevel=2,
        )
        return c_find_allocation_position(
...
class SchedulingEnvCacheOptimized(SchedulingEnv):
    ...
    def find_allocation_position(self, action):
        ...
        # C言語実装で位置を探索（クラウドは連続矩形のみ）
        position, waiting_time = _call_find_allocation_position(
            cache_c, job_width, job_height, when_submitted, current_time, use_cloud
        )
    ...
    def time_transition(self, slide_on_premise=True, slide_cloud=True):
        ...
        if slide_on_premise:
            ...
            if getattr(self, "_record_history", True):  # 評価時は env._record_history=False で履歴skip(512GiB回避)
        ...
        while True:
            ...
            if position is None:
                if np.all(allocated_job == 0):
                    ...
                    # 実時間はオンプレ・クラウドで共有されるため、待機中も両方の資源状態を進める
                    self.time_transition(True, True)
                    # time_transitionで差分更新済みなので、再構築不要
                    # ただし、次回アクションで異なるuse_cloudの場合は再構築が必要
                    use_cloud_initial = None
    ...
    def finalize_window_history(self):
        ...
        # 直前の time_transition で履歴に積んだ列と chrono 先頭が同一になることがある。
        # そのまま連結すると時刻列が1本重複し、可視化で継ぎ目付近が不自然に見える。
        if hist_onpre.shape[1] > 0 and np.array_equal(
            hist_onpre[:, -1], window_onpre_chrono[:, 0]
        ):
            window_onpre_chrono = window_onpre_chrono[:, 1:]
        if hist_cloud.shape[1] > 0 and np.array_equal(
            hist_cloud[:, -1], window_cloud_chrono[:, 0]
        ):
            window_cloud_chrono = window_cloud_chrono[:, 1:]
