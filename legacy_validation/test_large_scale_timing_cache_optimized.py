#!/usr/bin/env python3
"""
数千ジョブでの割り当て時間計測スクリプト（キャッシュ最適化版）
キャッシュ再構築ロジックを最適化した環境を使用してパフォーマンスを測定
"""
import sys
import os
import numpy as np
...
def run_environment_timing_test(nb_jobs: int,
    ...
    """環境の時間計測テストを実行（キャッシュ最適化版）"""
    print(f"\n{'='*60}")
    print(f"環境時間計測テスト開始（キャッシュ最適化版）: {nb_jobs}ジョブ")
...
def print_timing_results(results: Dict[str, Any]):
    """時間計測結果を表示"""
    print(f"\n{'='*60}")
    print("時間計測結果（キャッシュ最適化版）")
    print(f"{'='*60}")
    
    print(f"ジョブ数: {results['nb_jobs']:,}")
    print(f"実装: キャッシュ最適化版")
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='数千ジョブでの割り当て時間計測（キャッシュ最適化版）')
    ...
    parser.add_argument('--profile_output', type=str, default='profile_output_cache_optimized.prof',
