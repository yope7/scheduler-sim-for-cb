#!/usr/bin/env bash
# trace1024 を trace256 と同条件(FILM+Fourier)で学習し各iteration ckptを保存(振動のスケール依存を見る)。
set -u
cd /home/noguchi/scheduler-sim-for-cb
NITER="${1:-100}"
CFG=experiments/distributed_pcn/job_trace_1024_pcn.yml
OUT="experiments/distributed_pcn/run_trace1024_fourier_lc"
rm -rf "$OUT"; mkdir -p "$OUT"
echo "[trace1024_lc] NITER=$NITER OUT=$OUT START=$(date +%H:%M:%S)"
PCN_FAST_UPDATE=1 \
PCN_FOURIER_CMD=1 PCN_FOURIER_BANDS=4 PCN_FILM=1 \
DISTRIBUTED_PCN_CONFIG=$CFG DISTRIBUTED_PCN_JOBS=1024 DISTRIBUTED_PCN_OUTPUT_DIR=$OUT \
...
DISTRIBUTED_PCN_INITIAL_EPISODES=32 DISTRIBUTED_PCN_EVAL_INTERVAL=10 DISTRIBUTED_PCN_EVAL_SAMPLES=16 \
...
echo "[trace1024_lc] DONE=$(date +%H:%M:%S) exit=$?"
