#!/usr/bin/env bash
# trace256 の振動対策レバー汎用ランチャ: run_trace256_lc.sh と全く同条件に、
# 任意の LEVER 環境変数だけを足して学習する。iteration ckpt を保存(EMA時はEMA重み)。
# 使い方: scripts/run_trace256_lever.sh <TAG> "<LEVER env string>" [NITER]
#   例: scripts/run_trace256_lever.sh lrcos "PCN_LR_DECAY=cosine PCN_LR_DECAY_FINAL=0.1"
set -u
...
OUT="experiments/distributed_pcn/run_trace256_${TAG}"
rm -rf "$OUT"; mkdir -p "$OUT"
echo "[trace256_$TAG] NITER=$NITER LEVER=[$LEVER] OUT=$OUT START=$(date +%H:%M:%S)"
env $LEVER \
PCN_FAST_UPDATE=1 \
...
echo "[trace256_$TAG] DONE=$(date +%H:%M:%S) exit=$?"
