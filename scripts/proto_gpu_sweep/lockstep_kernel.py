"""lockstep_kernel — Phase3(NN方策rollout)向けのロックステップ GPU カーネル(B-1 + B-2)。

設計書: scripts/proto_gpu_sweep/lockstep_design.md

B-1(stepカーネル分割): raw_rollout_kernel._build_kernel の「for j in range(T)」内部ループを
外へ出し、`step_kernel` = 「全エピソードの j 番目のジョブを1個配置」する単発カーネルに再構成した。
状態(イベント表・order・count・prune閾値・累積時刻)は alloc_state() が確保する GPU 常駐の
デバイス配列に持たせ、ホストで j=0..T-1 を回して都度カーネルを起動する(run_lockstep_rollout)。
既存 raw_rollout_kernel.run_raw_rollout は一切変更していない(Phase1経路は不変)。

B-2(obs構築カーネル): 各 step 開始時(job j を配置する直前)の観測 obs[B,224] を GPU 上で
構築する `obs_kernel`。リファレンスは event_c_env.SchedulingEnvEventObs.get_observation
(event_native_env が呼ぶもの)。このカーネルは以下の**固定**環境変数構成のみを実装する
(replay_obs_builder / verify_replay_obs.py と同一構成):
    SCHEDULER_OBS_URGENCY=1, SCHEDULER_OBS_EFFICIENCY=1,
    SCHEDULER_OBS_OCCUPANCY=0, SCHEDULER_OBS_BUDGET_RATIO=0, SCHEDULER_ORACLE_NPZ=""
(一般の環境変数組み合わせには対応しない。B-3 以降で必要になれば拡張する。)

観測の要点(get_observation の定義を正確に写す):
  - events 部分(30イベント×6特徴): 「そのジョブが処理される直前」の event_buf 全体
    (資源ごとの prune された _resource_events とは別物。SchedulingEventBuffer は
    エピソード全体で一切 prune されない)から end>=window_start を残し、start 昇順の
    安定ソートで並べ、末尾(=startが大きい方)から最大30件を取る。
    ev_all[t] は「job t の配置結果」そのもの(このコードでは defer 非対応なので
    ev_all の t 番目 = job t、job 到着順=配置順=イベント挿入順が一致する)。
  - job_queue 部分(5ジョブ×8列): jobs[j:j+5] を _to_queue_job(=np.roll(raw,-1)) した生値
    (SCHEDULER_OBS_JQ_NORM 既定OFFなので無正規化のまま)。
  - urgency(1次元): 現ジョブをオンプレに「今すぐ」置いたときの開始時刻 s_on を求める
    副作用なしクエリ(find_start、continuous_only=False)。pw=max(0,s_on-arrival)、
    clip(log1p(pw)/16, 0, 1)。
  - efficiency(3次元): a=pt*nodes(現ジョブの雲送りコスト)、b=max(0,s_on-s_cl)
    (s_cl は同じジョブをクラウドに置いたときの開始時刻、continuous_only=True)。
    [clip(log1p(a)/20,0,1), clip(log1p(b)/16,0,1), clip((log1p(b)-log1p(a))/40+0.5,0,1)]。

find_start の副作用について: 配置探索(_find_event_allocation)は「償却 prune」で資源別
イベント配列を前詰め圧縮する(結果に不変だが状態は変える)。urgency/efficiency のクエリも
CPU 参照実装では同じ _find_event_allocation を呼ぶため同じ prune を起こしうる。この GPU 実装は
prune を含む探索一式を `_find_start` device 関数に切り出し、urgency 探索・efficiency 探索・
実配置探索の 3 箇所すべてから独立に呼ぶ(CPU 参照の「_front_alloc_cache 経由でオンプレ探索を
1回に統合する」高速化とは異なる実装だが、探索は状態の純粋関数(count 系は各呼び出しの終わりに
必ずゼロへ巻き戻す)なので、独立に複数回呼んでも数値結果・持続する状態(n_ev/prune_at)は
CPU 参照のキャッシュ再利用パスと完全に一致する。設計書の「find_start(use_cloud=False)として
関数化し再利用する」はこの独立呼び出し方式で満たされる)。

タイミングの対応関係: get_observation() は「次に処理する job j」を処理する直前に呼ばれる
(reset() では job 0、step() 内では自ジョブ処理後に次の観測として返る)。よって
obs_kernel(j) は job j-1 まで配置し終えた状態(time_state, event 履歴)を読み、job j を
まだ配置していない状態で観測を作る。ホストループは必ず obs_kernel(j) → step_kernel(j) の
順で呼ぶこと。
"""
...
def _build_step_kernel():
    ...
    def _kernel(
        ...
        if not reuse_hit:
            best_start, best_nfrag, n_ev_r, prune_at_r, ovf_code = _find_start(
                e, arrival, width, height, use_cloud, n_nodes, thr,
                n_ev_r, prune_at_r,
                ev_start_r, ev_end_r, ev_nfrag_r, ev_frag_lo_r, ev_frag_hi_r,
                order_end_r, order_start_r, counted_r, count_r,
                remap, pick_lo, pick_hi,
...
def _build_obs_kernel():
    ...
    def _kernel(
        ...
        n_ev_r = n_ev_on_arr[e]
        prune_at_r = prune_at_on_arr[e]
        s_on, nf_u, n_ev_r, prune_at_r, ovf_code = _find_start(
            e, arrival, width, height, False, n_on, thr,
            n_ev_r, prune_at_r,
            ev_on_start, ev_on_end, ev_on_nfrag, ev_on_frag_lo, ev_on_frag_hi,
            order_end_on, order_start_on, counted_on, count_on,
            remap, pick_lo, pick_hi,
        ...
        s_cl, _nf_e, n_ev_rc, prune_at_rc, ovf_code2 = _find_start(
            e, arrival, width, height, True, n_cl, thr,
            n_ev_rc, prune_at_rc,
            ev_cl_start, ev_cl_end, ev_cl_nfrag, ev_cl_frag_lo, ev_cl_frag_hi,
            order_end_cl, order_start_cl, counted_cl, count_cl,
            remap, pick_lo, pick_hi,
