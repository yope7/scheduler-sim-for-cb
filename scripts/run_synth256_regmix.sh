#!/usr/bin/env bash
# レジーム混合学習: synth256 を PCN_MIX_REGIMES(到着スケール=混雑ρの制御)を散らして学習。
# 混雑レジームが無次元PF形の支配因子なので、これを散らして「多様レジームに強いモデル」を狙う。
# 使い方: scripts/run_synth256_regmix.sh <TAG> "<REGIMES>" [NITER] [EXTRA env]
#   例: scripts/run_synth256_regmix.sh rm1 "1,20,175" 100 "CUDA_VISIBLE_DEVICES=0"
set -u
cd /home/noguchi/scheduler-sim-for-cb
TAG="${1:?set TAG}"; REGIMES="${2:?set REGIMES e.g. 1,20,175}"; NITER="${3:-100}"; EXTRA="${4:-}"
CFG=experiments/distributed_pcn/job_synthetic_pcn.yml
OUT="experiments/distributed_pcn/run_synth256_${TAG}"
rm -rf "$OUT"; mkdir -p "$OUT"
echo "[regmix] TAG=$TAG REGIMES=[$REGIMES] NITER=$NITER START=$(date +%H:%M:%S)"
env $EXTRA \
PCN_MIX_REGIMES="$REGIMES" \
PCN_FAST_UPDATE=1 PCN_FOURIER_CMD=1 PCN_FOURIER_BANDS=4 PCN_FILM=1 \
...
PCN_TRAIN_PF_BALANCE_REF=32 PCN_TRAIN_PF_BALANCE_ALPHA=0.5 \
...
echo "[regmix] TAG=$TAG DONE=$(date +%H:%M:%S) exit=$?"
