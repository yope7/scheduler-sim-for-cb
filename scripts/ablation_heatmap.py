#!/usr/bin/env python3
"""3機能アブレーション表をスライド用ヒートマップ化。HV/乖離/Spacingを『良さ』で色付け(緑=良),
n_pfは色なし。日本語は Noto Serif CJK JP。出力 docs/figures/ablation_heatmap.png
"""
import numpy as np
...
# (F, S, K), HV, HVstd, 乖離, 乖離std, Spacing, n_pf
rows = [
...
fig.suptitle("3機能アブレーション ヒートマップ  （緑=良い / 赤=悪い ・ n_pf は色なし）", fontsize=14, weight="bold", y=0.99)
fig.text(0.5, 0.015, "最良＝全機能ON（HV 0.825・乖離 0.095）。Fourier と サンプリング動的調整が効き、組合せで最良。", ha="center", fontsize=10, color="#444")
fig.tight_layout(rect=[0, 0.03, 1, 0.96])
fig.savefig("docs/figures/ablation_heatmap.png", dpi=130, bbox_inches="tight")
print("[SAVED] docs/figures/ablation_heatmap.png")
