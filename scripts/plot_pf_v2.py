...
"""実用版early-stop の最終判定: 4群を同一土俵で比較。
  ...
  practical-v2: es 各runを DISTRIBUTED_PCN_EARLYSTOP=1 で新規学習し best_model.pth を採用 (実用版)
共通真PF/cmax/ref = 全群 union。実用版が oracle に届くか(平均84%/std10%/効率3/5)を見る。
出力: docs/figures/pf_512_pf_v2.png
...
G = {"baseline": load_group("repro"), "oracle-ES": load_group("estop"), "practical-v2": load_group("esnew")}
for k, v in G.items():
...
COL = {"baseline": "#c0c4cc", "oracle-ES": "#9334e6", "practical-v2": "#1a73e8"}
...
for name in ["baseline", "practical-v2"]:
    for k, (i, (g, _)) in enumerate(G[name].items()):
        o = np.argsort(g[:, 0])
        axL.plot(g[o, 0] / cmax, g[o, 1], "-o" if name == "practical-v2" else "-",
                 c=COL[name], lw=1.4 if name == "practical-v2" else 1.0, ms=3,
                 alpha=.85 if name == "practical-v2" else .55,
                 zorder=3 if name == "practical-v2" else 1,
...
x = np.arange(5)
for j, name in enumerate(["baseline", "oracle-ES", "practical-v2"]):
...
names = ["baseline", "oracle-ES", "practical-v2"]
...
fig.suptitle("512 trace — practical early-stop v2 (all-candidate nadir) achieved PFs vs baseline",
             fontsize=12.5, y=1.02, fontweight="bold")
fig.savefig("docs/figures/pf_512_pf_v2.png", dpi=125, bbox_inches="tight")
print("\nsaved docs/figures/pf_512_pf_v2.png")
