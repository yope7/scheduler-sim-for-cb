...
使用方法:
  uv run python scripts/scale_benchmark.py              # ルートのシム（実体は benchmarks/）
  uv run python scripts/benchmarks/scale_benchmark.py --scale large
  uv run python scripts/benchmarks/scale_benchmark.py --output results.json
"""
import argparse
import json
import os
